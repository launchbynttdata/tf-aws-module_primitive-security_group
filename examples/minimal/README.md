# Example: Minimal Security Group Configuration

This example demonstrates the absolute minimum configuration required to create an AWS Security Group using the primitive module.

## Features

- Creates a VPC for context
- Configures the default security group to deny all traffic (security best practice)
- Creates a security group with only required parameters
...
| <a name="input_logical_product_service"></a> [logical\_product\_service](#input\_logical\_product\_service) | Logical product service name | `string` | `"minimal"` | no |
...
| <a name="input_tags"></a> [tags](#input\_tags) | A map of tags to add to all resources | `map(string)` | <pre>{<br/>  "Environment": "dev",<br/>  "Example": "minimal"<br/>}</pre> | no |
